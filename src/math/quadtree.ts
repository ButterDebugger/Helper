...
export class BoundingCircle<T> implements BoundingShape<T> {
    ...
    contains(point: Point<T>): boolean {
        const dist = Math.pow(point.x - this.x, 2) +
    ...
    intersects(rect: BoundingRectangle<T>): boolean {
        ...
        const edges = Math.pow(xDist - rect.width, 2) +
            Math.pow(yDist - rect.height, 2);

        if (
            xDist > this.radius + rect.width ||
            yDist > this.radius + rect.height
        ) return false;
...
export class QuadTree<T> {
    ...
    private subdivide(): void {
        ...
        this.topRight = new QuadTree(
            new BoundingRectangle(x + w, y - h, w, h),
            this.capacity,
        );
